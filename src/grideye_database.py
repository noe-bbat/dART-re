...
class GridEyeDatabase:
    ...
    def load_data(csv_path):
        ...
        try:
            ...
            df['avg_temp'] = df.iloc[:, 2:].mean()  # Calculate average temperature
            return df
        except Exception as e:
    ...
    def display_csv_data(self, csv_path):
        ...
        if df is not None:
            ...
            fig = px.line(filtered_df, x='timestamp', y='avg_temp',
            ...
            if st.checkbox("Show real time data", key=f"show_rt_{csv_path}"):
                npoints = 500
    ...
    def run(self):
        ...
        try:
            ...
            if not csv_files:
                st.warning("No GrideEye sensor data files found.")
                return
            tabs = st.tabs([os.path.basename(f) for f in self.csv_files])
