...
void Client::connect(const Address &address, std::chrono::seconds timeout)
{
    std::promise<void> connectionPromise;
    std::future<void> connectionFuture = connectionPromise.get_future();

    // Start the connection process in a separate thread
    std::thread connectionThread([this, &address, &connectionPromise]() {
        try {
    ...
    if (connectionFuture.wait_for(timeout) == std::future_status::timeout) {
        connectionThread.detach();  // Detach the thread to avoid blocking
...
void Client::connect(const std::string &str, std::chrono::seconds timeout)
{
    std::promise<void> connectionPromise;
    std::future<void> connectionFuture = connectionPromise.get_future();

    // Start the connection process in a separate thread
    std::thread connectionThread([this, &str, &connectionPromise]() {
        try {
            client.connect(str);
            connectionPromise.set_value();
        } catch (...) {
...
void Client::connect(std::chrono::seconds timeout)
{
    ...
    // Start the connection process in a separate thread
    std::thread connectionThread([this, &connectionPromise]() {
        try {
...
void Client::disconnect()
   {
       ...
       std::future<void> disconnectFuture = disconnectPromise.get_future();

       std::thread disconnectThread([this, &disconnectPromise]() {
           try {
               for (const auto &descriptor : event_descriptors) {
               ...
               disconnectPromise.set_exception(std::current_exception());
           }
       });

       // Attendre la déconnexion avec un timeout
       if (disconnectFuture.wait_for(std::chrono::seconds(5)) == std::future_status::timeout) {
           disconnectThread.detach();  // Détacher le thread pour éviter le blocage
           throw std::runtime_error("Timeout de déconnexion");
       }
